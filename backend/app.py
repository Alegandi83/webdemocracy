...
async def reset_database(request: Request, db: Session = Depends(get_db)):
    """Reset and reinitialize database - ADMIN ONLY - USE WITH CAUTION"""
    from sqlalchemy import text
    from lakebase_connector import postgres_pool, initialize_schema
    ...
    if not user_db or user_db.user_role != "admin":
        raise HTTPException(status_code=403, detail="Solo gli admin possono resettare il database")
    
    try:
        # Drop schema
        with postgres_pool.begin() as conn:
            conn.execute(text("DROP SCHEMA IF EXISTS webdemocracy CASCADE"))
        
        # Reinitialize schema
        initialize_schema()
        
        return {
            "message": "Database resettato e reinizializzato con successo",
            "details": "Schema webdemocracy droppato e ricreato con tutte le tabelle aggiornate"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Errore durante il reset del database: {str(e)}")
...
async def serve_spa_routes(full_path: str):
    ...
    # Don't intercept API, metrics, static, assets, tags, surveys, or settings routes
