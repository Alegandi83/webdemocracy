...
from lakebase_connector import get_db

# Import models with lakebase schema
# Schema initialization is handled by lakebase_connector

app = FastAPI(title="Web Democracy API (Databricks)", version="2.1.0")

# Get static directory path (for serving React frontend)
static_dir = os.path.join(os.path.dirname(__file__), "static")
print(f"Static directory: {static_dir}")
print(f"Static directory exists: {os.path.exists(static_dir)}")

# Configurazione CORS - Allow all origins for Databricks Apps
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def get_config_urls():
    """Restituisce gli URL di configurazione per il frontend"""
    # Databricks Apps fornisce DATABRICKS_HOST
    ...
    # Costruisci l'URL per Databricks One (aggiungi https:// se non presente)
    if databricks_host:
        if databricks_host.startswith('http://') or databricks_host.startswith('https://'):
            databricks_one_url = f"{databricks_host}/one"
        else:
            databricks_one_url = f"https://{databricks_host}/one"
    else:
        databricks_one_url = None
    
    return {
        "databricks_one_url": databricks_one_url,
...
async def debug_static_files():
    ...
    return {
        ...
        "files": sorted(static_files)[:100],  # First 100 files
        "note": "Only showing first 100 files"
    }

@app.get("/api/debug/database")
async def debug_database(db: Session = Depends(get_db)):
    """Debug endpoint to verify database schema"""
    import os
    
    # Raccogli variabili d'ambiente relative al database
    env_vars = {}
    ...
    try:
        ...
        result = db.execute(text("""
            ...
            WHERE schema_name = 'webdemocracy'
        """))
        schema_exists = result.fetchone() is not None
        
        # Verifica tabelle nello schema webdemocracy
        result = db.execute(text("""
            SELECT table_name 
            FROM information_schema.tables 
            WHERE table_schema = 'webdemocracy'
            ORDER BY table_name
        """))
        tables = [row[0] for row in result.fetchall()]
        
        # Conta record in user table
        user_count = 0
        if 'user' in tables:
            result = db.execute(text('SELECT COUNT(*) FROM webdemocracy."user"'))
        ...
        return {
            ...
            "database_ok": schema_exists and len(tables) > 0
        }
...
async def get_current_user_id(request: Request, db: Session = Depends(get_db)) -> int:
    """Get current user ID from Databricks headers and database"""
    user_email = request.headers.get("x-forwarded-email")
    if not user_email:
        user_email = "demo@local.dev"  # Fallback per test locali
...
async def get_current_user(request: Request, db: Session = Depends(get_db)):
    """Get current user information from Databricks Apps headers and database"""
    # Recupera gli header X-Forwarded-* forniti da Databricks Apps
    # Riferimento: https://docs.databricks.com/aws/en/dev-tools/databricks-apps/http-headers
    user_name = request.headers.get("x-forwarded-preferred-username")
    user_id_header = request.headers.get("x-forwarded-user")
    user_email = request.headers.get("x-forwarded-email")
    user_ip = request.headers.get("x-real-ip") or (request.client.host if request.client else "127.0.0.1")
    
    # Se non ci sono header (ambiente locale), usa utente di test
    ...
    return {
        ...
        "is_local": not (user_name or user_id_header),
        "is_admin": is_admin,
...
async def update_tag(tag_id: int, tag_update: schemas.TagUpdate, request: Request, db: Session = Depends(get_db)):
    ...
    user_email = request.headers.get("x-forwarded-email", "demo@local.dev")
    user_db = db.query(models.User).filter(models.User.email == user_email).first()
...
def get_setting(key: str, db: Session = Depends(get_db)):
    ...
    # Ottieni il valore di default da DATABRICKS_APP_URL per qr_code_url
    databricks_app_url = os.getenv("DATABRICKS_APP_URL", "https://example.com")
    
    if not setting:
        # Ritorna valore di default se non esiste nel database
        if key == "qr_code_url":
            # Usa DATABRICKS_APP_URL come default iniziale
            return {"key": key, "value": databricks_app_url}
        raise HTTPException(status_code=404, detail="Setting non trovato")
    
    # Per qr_code_url, migra automaticamente valori di default/test al valore corretto
    if key == "qr_code_url" and setting.value:
        # Lista di valori da migrare automaticamente
        default_values = ["localhost", "example.com", "databricks.com", "http://", "127.0.0.1"]
        should_migrate = any(val in setting.value.lower() for val in default_values)
        
        # Se il valore contiene uno dei valori di default E abbiamo DATABRICKS_APP_URL, aggiorna
        if should_migrate and databricks_app_url != "https://example.com":
            setting.value = databricks_app_url
            db.commit()
            return {"key": setting.key, "value": databricks_app_url}
    
    # Se esiste nel database, usa quel valore (permette personalizzazione dagli admin)
...
async def reset_setting(key: str, request: Request, db: Session = Depends(get_db)):
    """Resetta un setting al valore di default - Admin only"""
    # Verifica che l'utente sia admin
    user_email = request.headers.get("x-forwarded-email", "demo@local.dev")
    user_db = db.query(models.User).filter(models.User.email == user_email).first()
    
    if not user_db or user_db.user_role != "admin":
        raise HTTPException(status_code=403, detail="Solo gli admin possono resettare i settings")
    
    # Elimina il setting dal database (così verrà usato il valore di default)
    setting = db.query(models.Settings).filter(models.Settings.key == key).first()
    if setting:
        db.delete(setting)
        db.commit()
    ...
    if key == "qr_code_url":
        default_value = os.getenv("DATABRICKS_APP_URL", "https://example.com")
        return {"key": key, "value": default_value, "message": "Setting resettato al valore di default"}
    
    return {"key": key, "value": None, "message": "Setting eliminato"}
...
async def serve_favicon():
    ...
    full_path = os.path.join(static_dir, "favicon.svg")
...
async def serve_assets(file_path: str):
    ...
    if os.path.exists(full_path) and os.path.isfile(full_path):
        # Use mimetypes to get correct Content-Type
...
async def serve_static_files(file_path: str):
    """Serve static files (JS, CSS, images, etc.) - handles all nested paths"""
    # React build mette le immagini in static/static/media/
    # Prima prova il path diretto, poi prova con static/ prefisso
    full_path = os.path.join(static_dir, file_path)
    
    if not os.path.exists(full_path) or not os.path.isfile(full_path):
        # Se il file non esiste nel path diretto e inizia con "media/", 
        # prova in static/media/ (React build structure)
        if file_path.startswith("media/"):
            full_path = os.path.join(static_dir, "static", file_path)
    
    if os.path.exists(full_path) and os.path.isfile(full_path):
        # Use mimetypes to get correct Content-Type for all file types
        media_type, _ = mimetypes.guess_type(full_path)
        if file_path.endswith('.svg') and not media_type:
            media_type = "image/svg+xml"
        elif file_path.endswith('.png') and not media_type:
            media_type = "image/png"
        elif file_path.endswith('.jpg') and not media_type:
            media_type = "image/jpeg"
        return FileResponse(full_path, media_type=media_type)
    
    # Log only 404s for debugging
    print(f"❌ Static file not found: /static/{file_path}")
    raise HTTPException(status_code=404, detail=f"File not found: {file_path}")


# ============================================================================
# H3 Geospatial Map Endpoints (using SQL Warehouse, not Lakebase)
# ============================================================================

@app.get("/api/map/h3-data")
async def get_h3_map_data(
    request: Request,
    resolution: int = Query(6, description="H3 resolution (1-15)"),
    catalog: str = Query("dad_open_data", description="Catalog name"),
    schema: str = Query("locations", description="Schema name"),
    table: str = Query("world", description="Table name"),
    column: str = Query("cities_h3_res_15", description="H3 column name"),
    ...
    """
    Get H3 hexagon data for map visualization using SQL Warehouse
    
    Args:
        request: FastAPI Request for OAuth token
        resolution: H3 resolution level (1-15) - user selected
        catalog, schema, table, column: Table and column to query
        min_lat, max_lat, min_lng, max_lng: Viewport bounds (optional)
    
    Returns:
        List of hexagons with GeoJSON boundaries and counts
    """
    import json
    import asyncio
    from concurrent.futures import ThreadPoolExecutor
    from shapely.geometry import Polygon
    from sql_warehouse_connector import execute_sql_warehouse_query
    
    # Use parameters from request (dynamic table selection)
    # Resolution is used as the second parameter in h3_toparent(column, resolution)
    
    try:
        # Build bounds filter if provided
        bounds_filter = ""
        if all([min_lat is not None, max_lat is not None, min_lng is not None, max_lng is not None]):
            # Create WKT polygon for bounds
            polygon_coords = [
                (min_lng, min_lat),  # lower left
                (min_lng, max_lat),  # upper left
                (max_lng, max_lat),  # upper right
                (max_lng, min_lat),  # lower right
                (min_lng, min_lat)   # close polygon
            ]
            poly = Polygon(polygon_coords)
            bounds_wkt = poly.wkt
            bounds_filter = f"AND h3_toparent({column}, {resolution}) IN (SELECT EXPLODE(H3_COVERASH3('{bounds_wkt}', {resolution})))"
        
        # Query for H3 data using SQL Warehouse
        query = f"""
            WITH cell_agg AS (
                SELECT
        ...
        # Execute query on SQL Warehouse (not Lakebase) with timeout
        # SQL Warehouse may take 2-3 minutes to start if stopped
        print("Starting query execution with 180s timeout...")
        
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as pool:
            try:
                # Run the blocking SQL query in a thread with 3 minute timeout
                df = await asyncio.wait_for(
                    loop.run_in_executor(pool, execute_sql_warehouse_query, query, request),
                    timeout=180.0  # 3 minutes for warehouse to start and execute query
                )
                print(f"Query completed successfully, processing {len(df)} rows")
            except asyncio.TimeoutError:
                print("ERROR: Query timed out after 180 seconds")
                raise HTTPException(
                    status_code=504, 
                    detail="Query timeout: SQL Warehouse took too long to respond. It may be starting up. Please try again in 1-2 minutes."
                )
        
        # Process results
        data = []
        for _, row in df.iterrows():
            hex_boundary = json.loads(row['hex_boundary']) if isinstance(row['hex_boundary'], str) else row['hex_boundary']
            data.append({
                "hex_boundary": hex_boundary,
                "count": int(row['count'])
            })
        
        print(f"Returning {len(data)} hexagons")
        return {
            "data": data,
        ...
        import traceback
        print(f"Error fetching H3 data: {str(e)}")
        print(traceback.format_exc())
        raise HTTPException(status_code=500, detail=f"Error fetching H3 data: {str(e)}")


@app.get("/api/map/config")
async def get_map_config():
    """Get map configuration (table, column, default settings)"""
    return {
...
async def serve_spa_routes(full_path: str):
    ...
    # Don't intercept API, metrics, static, assets, media, tags, surveys, or settings routes
    if (full_path.startswith("api/") or 
        full_path == "metrics" or 
        full_path.startswith("static/") or
        full_path.startswith("assets/") or
        full_path.startswith("tags") or
        full_path.startswith("surveys") or
        full_path.startswith("settings")):
    ...
    return HTMLResponse(
        content="<html><body><h1>Frontend not built</h1><p>Run ./build.sh to build the frontend</p></body></html>",
        status_code=200
    )
